...
class GameController extends Controller
{
    ...
    public function join(GameJoinRequest $request, Game $game): JsonResponse|GameJoinResource
    {
        ...
        if (empty($inGame)) {
            ...
            // Si le joueur est déjà dans la partie, on le récupère
            $gamePlayer = $inGame;
            event(new JoiningPlayerEvent($gamePlayer));
        }
        ...
        try {
            ...
            if (empty($request->selectedThemes)) {
                ...
                for ($i = 0; $i < $request->question_count; $i++) {
                    ...
                    $questionToAddId = Question::where('is_integrated', true)->whereNotIn(
                        'id',
                        $gameQuestionsIds
                ...
                for ($i = 0; $i < $request->question_count; $i++) {
                    ...
                    if (!$request->allTags) {
                        ...
                        })->where('is_integrated', true)->whereNotIn('id', $gameQuestionsIds)->inRandomOrder()->first(
                        )->id;
                        ...
                        $questionToAddId = Question::whereHas('tags', function (Builder $query) use ($tagIds) {
                            ...
                            'is_integrated',
                            true
                        )->whereNotIn('id', $gameQuestionsIds)->inRandomOrder()->first()->id;
                    }
                    // Attribution de la question à la partie
                    GameQuestion::create([
                                             'game_id'     => $newGame->id,
        ...
        if ($game->has_begun) {
            ...
            if (empty($gamePlayer)) {
                return response()->json(
                    ['success' => false, 'message' => "Vous n'êtes pas autorisé à intégrer la partie"],
                    500
                );
            }
        }
    ...
    public function begin(GameBeginRequest $request): JsonResponse
    {
        ...
        try {
            ...
            for ($i = 0; $i < $game->question_count; $i++) {
                ...
                elseif (!$game->questions_have_all_tags) {
                    $randomQuestionId = Question::with([
                                                           'tags' => function ($query) use ($gameTags) {
                    ...
                    $randomQuestionId = Question::withCount([
                                                                'tags' => function ($query) use ($gameTags) {
                                                                    $query->whereIn('tag_id', $gameTags);
                                                                },
                                                                '>=',
