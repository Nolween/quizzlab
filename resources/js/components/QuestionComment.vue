<template>
    <div class="bg-white mb-3">
        ...
        <!-- Actions -->
        <div v-if="props.responseDisplay == false" class="flex flex-wrap justify-end pr-3 space-x-3 pb-3">
            ...
            <span  v-if="userStore.isConnected == true"  title="Répondre à ce commentaire">
                <svg-icon
                    :path="mdiCommentArrowRight"
                    class="text-quizzlab-quaternary w-7 h-7 cursor-pointer"
                    type="mdi"
                    @click="commentStore.replyComment(props.id)"
                ></svg-icon>
            </span>
        </div>
